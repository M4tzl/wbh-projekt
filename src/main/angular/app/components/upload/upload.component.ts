...
export class UploadComponent {
    @Input()
    inseratBild: InseratBild = <InseratBild> {};

    @Output()
    imageUploaded: EventEmitter<InseratBild> = new EventEmitter();

    get imageSource(): string {
        console.log(this.inseratBild.bildKey);
        return this.inseratBild.bildKey ? `/api/inserate/${this.inseratBild.inseratId}/images/${this.inseratBild.bildKey}`
    ...
    handleFileInput(files: FileList) {
        ...
        this.inserateService.uploadImage(this.inseratBild, fileToUpload)
            .pipe(
                tap(result => this.imageUploaded.emit(result))
            )
            .subscribe(result => this.inseratBild = result);
    }
}
